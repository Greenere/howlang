...
```bash
cmake --build c_interpreter/build
./c_interpreter/build/howlang --check path/to/file.how
./c_interpreter/build/howlang --dis path/to/file.how
./c_interpreter/build/howlang --compile path/to/file.how
```

For regressions, always also run:

```bash
./c_interpreter/build/howlang samples/tests/test_all.how
```
