...
static void die(const char *fmt, ...) {
    ...
    fprintf(stderr, "\033[31m[RuntimeError] ");
...
static TokenList *lex(const char *src) {
    ...
    while (pos < n) {
        ...
        if (isdigit(c)) {
            ...
            if (len >= 63) die("number too long at line %d", line);
        ...
        if (isalpha(c) || c == '_') {
            ...
            int len = pos - start;
            char tmp[256];
            if (len >= 255) die("identifier too long at line %d", line);
            memcpy(tmp, src+start, len); tmp[len] = 0;
...
#define TWO(a,b,r) if (pos < n && src[pos]==(b)) { pos++; t.type=(r); tl_push(tl,t); continue; }
        ...
        default: die("unexpected character '%c' at line %d", c, line);
        }
#undef TWO
        tl_push(tl, t);
    }
    Token eof = {TT_EOF, NULL, 0, 0, line};
    tl_push(tl, eof);
    return tl;
}
...
static Token *p_expect(Parser *p, TT t, const char *msg) {
    if (!p_check(p,t)) {
        ...
        die("%s (line %d, got token type %d)", msg, cur->line, cur->type);
    }
    return p_adv(p);
}
...
static Node *parse_atom(Parser *p) {
    ...
    die("unexpected token type %d at line %d", t, line);
    return NULL;
}
...
static void repl(Env *env) {
    printf("Howlang C interpreter  |  Ctrl-D to exit\n");
    char buf[16384];
    while (1) {
        ...
        if (!buf[0]) continue;
        /* wrap in a way that prints the result if it's an expr */
        run_source(buf, env);
    }
}
